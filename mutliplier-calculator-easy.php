<?php
/* For manually calculating multipliers for n-spot games, input necessary values in the 3rd and 4th lines of code! */
function calculateMultipliers($n = 1, $a = 40){ /* Instead of $n = 1 and $a = 40, input values that correspond with values you want to calculate multipliers for. */
    ...
    $validProbSum = 0;
	$multiplierCap = ($a == 40) ? 1000 : 2500000;
    ...
    foreach ($P as $hits => $probability){
        ...
        if ($multiplier < 1){
            ...
            $rawMultipliers[$hits] = min($multiplier, $multiplierCap);
    ...
    $finalMultipliers = array_map(function($multiplier) use ($SF, $multiplierCap){
        if ($multiplier < 1){
            return 0.0000;
        } else {
            $scaledMultiplier = $multiplier * $SF;
            return min(round($scaledMultiplier, 4), $multiplierCap);
        }
    }, $rawMultipliers);
    $previousMultiplier = 0;
    ...
    if (abs((1 - $expectedValue) * 100 - 0.75) > 0.0001){
        ...
        $finalMultipliers = array_map(function($multiplier) use ($correction, $multiplierCap){
            return ($multiplier >= 1) ? min(round($multiplier * $correction, 4), $multiplierCap) : $multiplier;
        }, $finalMultipliers);
        $expectedValue = array_sum(array_map(function($probability, $multiplier){
            return $probability * $multiplier;
        }, $P, $finalMultipliers));
...
foreach ($result['multipliers'] as $hits => $multiplier){
    echo sprintf("%4d match(es) | %8.4fx\n", $hits, $multiplier);
}
?>